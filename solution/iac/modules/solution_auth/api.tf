...
# Configure token to emit roles (not group GUIDs)
# resource "azuread_application_optional_claims" "solution_api" {
#   application_id = azuread_application_registration.solution_api.id
...
# # The following is saying that the the role is assigned to the group for a specific application.
# resource "azuread_app_role_assignment" "solution_api_group" {
#     for_each = var.solution_roles

#     app_role_id = azuread_application_app_role.solution_api_role[each.key].role_id
#     resource_object_id = azuread_service_principal.solution_api.object_id
#     principal_object_id = azuread_group.solution_api_role_group[each.key].object_id
# }

# resource "azuread_group_member" "solution_client_app_role" {
#   for_each = var.solution_roles

#   group_object_id  = azuread_group.solution_api_role_group[each.key].object_id
#   member_object_id = azuread_service_principal.solution_client_app.object_id
# }
